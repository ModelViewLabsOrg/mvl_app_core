...
class MaskTextInputFormatter implements TextInputFormatter {
  ...
  ///
  /// The keys of the [filter] assign which character in the
  ...
  ///  - [MaskAutoCompletionType.eager]: autocomplete
  /// unfiltered characters when the previous filtered character is input.
  ///  For example, with the mask "#/#" and the sequence of characters "1" then "2", the formatter will output "1/", then "1/2"
  ...
  TextEditingValue updateMask({
    ...
    var newEditingValue = newValue;
    if (newEditingValue == null) {
    ...
    return formatEditUpdate(TextEditingValue.empty, newEditingValue);
  }
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (beforeResultTextLength == 0 && _resultTextArray.length > 1) {
      ...
      if (prefixLength > 0) {
        final List<String> resultPrefix = _resultTextArray._symbolArray.take(prefixLength).toList();
        final int effectivePrefixLength = min(_resultTextArray.length, resultPrefix.length);
