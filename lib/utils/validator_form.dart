...
class FormValidator {
  ...
  String? password() {
    ...
    if (Validator(value).isPasswordValid()) return null;
  ...
  String? expDate() {
    ...
    try {
      ...
      if (split.first.length != 2 || split.last.length != 2) return error;

      final year = int.parse(split.last) + 2000;

      final diff = DateTime(year).differenceInYearsFromToday().abs();
  ...
  String? creditCardNumber() {
    ...
    if (length < 12 || length > 16) return 'Número inválido';
  ...
  String? expOnlyDay({bool acceptEmpty = false}) {
    ...
    if (acceptEmpty && (value == null || value.isReallyEmpty())) return null;

    if (value == null) return 'Dia inválido';

    final day = int.tryParse(value);
    if (day == null) return 'Dia inválido';

    if (day < 2 || day > 28) return 'Dia deve estar entre 2 e 28';
  ...
  String? nickname() {
    if ((value?.length ?? 0) > nicknameMaxChars) return 'Apelido muito grande';
  ...
  static String? birthday(DateTime? value, {bool checkIs18yrs = false}) {
    if (value == null) return birthDayError;
    final isValid = Validator('').isBirthdayValid(value);
    if (!isValid) return birthDayError;

    if (checkIs18yrs) {
      if (!value.is18yrs()) return 'Idade menor de 18 anos';
    }
  ...
  String? birthdayFromString({bool checkIs18yrs = false}) {
    ...
    final date = value.parseBrFormat('/');
  ...
  String? cpf() {
    final value = this.value.onlyNumbers();
  ...
  String? inviteCode() {
    ...
    if (length != 6) return 'O código deve ser de 6 dígitos';
  ...
  String? cvv() {
    final value = this.value;
    if (value == null) return 'CVV inválido';

    if (RegExp(r'\d{3,4}').hasMatch(value)) return null;
  ...
  String? zipcode() {
    final value = this.value;
    if (value == null) return 'Cep inválido';

    if (RegExp(r'\d{2}\.\d{3}-\d{3}').hasMatch(value)) return null;
