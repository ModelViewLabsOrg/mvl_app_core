...
import 'package:mvl_app_core/fcm_manager.dart';
...
class AppVersion {
  ...
  Future<void> init(EnvEnum env) async {
    this.env = env;
  ...
  Widget versionWidget(BuildContext context) {
    return GestureDetector(
      onLongPress: () async {
        final token = FCMManager.I().token;
        if (token == null) {
  ...
  Widget _versionChild(BuildContext context) {
    if (env.isPrd) {
      return AppText.titleLarge(versionStr, context);
    }

    return Column(
      children: [
        AppText.titleLarge(versionFull, context),
        gapM,
        AppText.titleLarge(env.name.toUpperCase(), context),
      ],
    );
  }
  ...
  JsonString toHeaders() {
    return {
      'AG_AppName': appName,
      'AG_version': versionFull,
      'AG_osName': osName,
      'AG_osVersion': osVersion,
    };
  }
}
