import 'package:flutter/foundation.dart';
import 'package:flutter/services.dart';
import 'package:mvl_app_core/app_config_values.dart';
import 'package:mvl_app_core/app_logger.dart';
import 'package:mvl_app_core/utils/env_enum.dart';

class AppConfig {
  factory AppConfig.I() {
    return _instance ??
        (throw UnimplementedError('Must call setup() before use it'));
  }

  AppConfig._internal(this.env);
  static late final AppConfig? _instance;

  final EnvEnum env;
  late final AppConfigValues configValues;

  static Future<AppConfig> setup(AppConfigValues values) async {
    return _instance = AppConfig._internal(await _env())..configValues = values;
  }

  static Future<EnvEnum> _env() async {
    String? flavor;
    try {
      const channel = MethodChannel('flavor');
      flavor = await channel.invokeMethod<String>('getFlavor');
    } catch (_) {
      // ignore: do_not_use_environment
      flavor = const String.fromEnvironment('flavor', defaultValue: 'uat');
    } finally {
      AppLogger.I().debug('Flavor: $flavor');
    }

    // if (flavor == null) return EnvEnum.uat;

    // return EnvEnum.values.firstWhere(
    //   (e) => flavor == e.name,
    //   orElse: () => EnvEnum.uat,
    // );
    return EnvEnum.prd;
  }

  static const trackingEnabled = !kDebugMode;
}
